window.onload = function(){
    ...
    function choicePos(){
        AMapUI.loadUI(['misc/PositionPicker'], function(PositionPicker) {
            ...
            positionPicker.on('success', function(positionResult) {
                document.querySelector('.indexPos').innerHTML = '[当前] '+positionResult.address; 
    ...
    function cityPickerReady(cityPicker) {
        ...
        AMapUI.load(['lib/$'], function($) {
            ...
            $(document).on('click', function(e) {
                ...
                //console.log(objSrc);
                if(objSrc === 'item J_city_item current_city'){
                    xuanzhong(2);
                } else{
                    xuanzhong(1);
                }
                function xuanzhong(difNum){
                    cityPicker.on('citySelected', function(cityInfo) {                                                  
                        //隐藏城市列表面板
                        cityPicker.hideImmediately();
                        //清除搜索框内容
                        cityPicker.clearSearch();
                        notifyCityInfo(cityInfo,difNum);
                    });
                }
        ...
        var marker = new AMap.Marker({
            ...
            /*显示对应的城市信息*/
            //jshint -W003
            function notifyCityInfo(cityInfo,difNum) {
                $("#selectedCityInfo").html(JSON.stringify(cityInfo, null, 2));
                marker.setMap(map);
                if(difNum === 2){
                    var localLng = sessionStorage.getItem("lng"); 
                    var localLat = sessionStorage.getItem("lat"); 
                    marker.setPosition([localLng, localLat]);
                } else {
    ...
    AMapUI.loadUI(['misc/MobiCityPicker'], function(MobiCityPicker) {
        ...
        //console.log(cityPicker.topGroups.id);
    ...
    map.plugin('AMap.Geolocation', function () {
        ...
        AMap.event.addListener(geolocation, 'complete',function(geolocationResult){
            sessionStorage.setItem("lng", geolocationResult.position.lng); 
            sessionStorage.setItem("lat", geolocationResult.position.lat); 
            sessionStorage.setItem("currentAddress", geolocationResult.formattedAddress); 
            map.setZoom(16);
            map.panBy(-5, 14);
        });//返回定位信息
    });
